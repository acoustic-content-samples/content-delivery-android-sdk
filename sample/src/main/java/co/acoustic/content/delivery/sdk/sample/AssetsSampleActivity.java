...
import co.acoustic.content.delivery.sdk.Asset;
import co.acoustic.content.delivery.sdk.Assets;
...
/**
 * The {@link BaseSampleActivity} implementation, demonstrates using  {@link ContentDeliverySDK} for working with Assets.
 */
public class AssetsSampleActivity extends BaseSampleActivity {
    /*
    ...
    /*
     * Assets instance which holds search parameters for initial query.
     * The could be updated on QueryBuilder Screen. See onQueryBuilderResultReady() method.
     */
    private Assets initialSearch;
    ...
    /*
     * Represents index of the first found Assets element. Used for displaying this index on Sample Screen.
     */
    private int firstLoadedItem = -1;
    /*
     * Represents index of the last found Assets element. Used for displaying this index on Sample Screen.
     */
     ...
     */
    private List<DeliverySearchResult<Asset>> loadedPages = new LinkedList<>();
    /*
     * Represents Asset results, converted to format understandable for RecyclerView.Adapter.
     */
    ...
    protected ContentDeliverySDK getContentDeliverySDK() {
        if (sdk == null) {
            ...
            initialSearch = deliverySearch.assets();
        }
        return sdk;
    }
    ...
    protected String getSampleTitle() {
        return getString(R.string.activity_sample_assets);
    }

    @NonNull
    @Override
    protected Class<?> getQueryBuilderScreenClass() {
        return AssetsQueryBuilderActivity.class;
    }
    ...
    protected void loadPrevPage() {
         ...
         */
        loadedPages.get(0).getState();
        Assets prevPage = loadedPages.get(0).previousPage();
        if (prevPage != null) {
            prevPage
                    .get()
    ...
    protected void loadNextPage() {
         ...
         */
        Assets nextPage = loadedPages.get(loadedPages.size() - 1).nextPage();
        nextPage
                .get()
                .then(this::handleNextPageResult)
                .error(this::handleError);
     ...
     *
     * @param result the {@link DeliverySearchResult<Asset>} instance.
     */
    private void handlePrevPageResult(DeliverySearchResult<Asset> result) {
        /*
         * Adding the result as first loaded page.
         */
        loadedPages.add(0, result);

        final List<Asset> items = result.getDocuments();
         ...
         */
        this.resultItems.addAll(0, parseAssets(firstLoadedItem, items));
     ...
     * @param result the {@link DeliverySearchResult<Asset>} instance.
     */
    private void handleNextPageResult(DeliverySearchResult<Asset> result) {
        /*
         ...
         *
         * NOTE: startIndex - is value that make sense only in scope of the sample, it is used only for adding indexes to ResultItem's. For more details see parseAssets() method.
         */
    ...
    /**
     * Parses {@link Asset} instances to {@link ResultItem}. For displaying in List.
     *
     * @param startIndex start position for items numbering.
     * @param source     the source list of {@link Asset}, that should be parsed.
     * @return the list of {@link ResultItem}.
     */
    private List<ResultItem> parseAssets(int startIndex, List<Asset> source) {
        int itemIndex = startIndex;
        final List<ResultItem> resultItems = new ArrayList<>(source.size());
        for (Asset item : source) {
            resultItems.add(new ResultItem(itemIndex++ + ". " + item.getName(), getAssetDetailsFormatterString(item)));
        }
        return resultItems;
    }

    private static String getAssetDetailsFormatterString(@NonNull Asset item) {
        ...
        return new StringBuilder()
                ...
                .append("assetType=").append(item.getAssetType()).append("\n")
                .append("categories=").append(item.getCategories()).append("\n")
                .append("categoryLeaves=").append(item.getCategoryLeaves()).append("\n")
                .append("fileSize=").append(item.getFileSize()).append("\n")
                .append("isManaged=").append(item.isManaged()).append("\n")
                .append("location=").append(item.getLocation()).append("\n")
                .append("locationPaths=").append(item.getLocationPaths()).append("\n")
                .append("media=").append(item.getMedia()).append("\n")
                .append("mediaType=").append(item.getMediaType()).append("\n")
                .append("path=").append(item.getPath()).append("\n")
                .append("resource=").append(item.getResource()).append("\n")
                .append("status=").append(item.getStatus()).append("\n")
                .append("thumbnail=").append(item.getThumbnail()).append("\n")
