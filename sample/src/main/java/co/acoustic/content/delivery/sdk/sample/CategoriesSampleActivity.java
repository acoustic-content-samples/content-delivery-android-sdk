...
import co.acoustic.content.delivery.sdk.Category;
...
/**
 * The {@link BaseSampleActivity} implementation, demonstrates using  {@link ContentDeliverySDK} for working with Categories.
 */
public class CategoriesSampleActivity extends BaseSampleActivity {
    /*
    ...
    /*
     * Categories instance which holds search parameters for initial query.
     * The could be updated on QueryBuilder Screen. See onQueryBuilderResultReady() method.
     */
    private Categories initialSearch;
    ...
    /*
     * Represents index of the first found Categories element. Used for displaying this index on Sample Screen.
     */
    private int firstLoadedItem = -1;
    /*
     * Represents index of the last found Categories element. Used for displaying this index on Sample Screen.
     */
     ...
     */
    private List<DeliverySearchResult<Category>> loadedPages = new LinkedList<>();
    /*
     * Represents Categories results, converted to format understandable for RecyclerView.Adapter.
     */
    ...
    protected ContentDeliverySDK getContentDeliverySDK() {
        if (sdk == null) {
            ...
            initialSearch = deliverySearch.categories();
        }
        return sdk;
    }
    ...
    protected String getSampleTitle() {
        return getString(R.string.activity_sample_categories);
    }

    @NonNull
    @Override
    protected Class<?> getQueryBuilderScreenClass() {
        return CategoriesQueryBuilderActivity.class;
    }
    ...
    protected void loadPrevPage() {
         ...
         */
        Categories prevPage = loadedPages.get(0).previousPage();
        if (prevPage != null) {
            prevPage
                    .get()
    ...
    protected void loadNextPage() {
         ...
         */
        Categories nextPage = loadedPages.get(loadedPages.size() - 1).nextPage();
        nextPage
                .get()
                .then(this::handleNextPageResult)
                .error(this::handleError);
     ...
     *
     * @param result the {@link DeliverySearchResult<Category>} instance.
     */
    private void handlePrevPageResult(DeliverySearchResult<Category> result) {
        /*
         * Adding the result as first loaded page.
         */
        loadedPages.add(0, result);

        final List<Category> items = result.getDocuments();
         ...
         */
        this.resultItems.addAll(0, parseCategories(firstLoadedItem, items));
    ...
    private void handleNextPageResult(DeliverySearchResult<Category> result) {
        /*
         ...
         *
         * NOTE: startIndex - is value that make sense only in scope of the sample, it is used only for adding indexes to ResultItem's. For more details see parseCategories() method.
         */
    ...
    /**
     * Parses {@link Category} instances to {@link ResultItem}. For displaying in List.
     *
     * @param startIndex start position for items numbering.
     * @param source     the source list of {@link Category}, that should be parsed.
     * @return the list of {@link ResultItem}.
     */
    private List<ResultItem> parseCategories(int startIndex, List<Category> source) {
        int itemIndex = startIndex;
        final List<ResultItem> resultItems = new ArrayList<>(source.size());
        for (Category item : source) {
            resultItems.add(new ResultItem(itemIndex++ + ". " + item.getName() + "\n"
                    + item.getId(), getCategoryDetailsFormatterString(item)));
        }
        return resultItems;
    }

    private static String getCategoryDetailsFormatterString(@NonNull Category item) {
