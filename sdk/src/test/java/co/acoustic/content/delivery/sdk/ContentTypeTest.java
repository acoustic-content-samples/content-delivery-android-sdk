...
public class ContentTypeTest {
    ...
    public void testCreation() {
        ContentType contentType = new ContentType(new DeliverySearchResponseDocument());
        assertNotNull("Constructor should create not null instance.", contentType);
    }

    @Test
    public void testToString() {
        ContentType document = new ContentType(createFullRawDoc());
        assertEquals(createExpectedToString(document), document.toString());
    }

    @Test
    public void testParcelable() {
        ContentType source = new ContentType(createFullRawDoc());

        Parcel parcel = MockParcel.obtain();
        source.writeToParcel(parcel, source.describeContents());
        parcel.setDataPosition(0);

        ContentType createdFromParcel = ContentType.CREATOR.createFromParcel(parcel);

        assertEquals("Builders should builder equals TestDocumentImpl instances ", source, createdFromParcel);
        final int testArraySize = 2;
        ContentType[] testArray = ContentType.CREATOR.newArray(testArraySize);
